...
def m_input_fn(FILE):
	def get_line(line):
		...
		return dict(zip(feature_names, line[0:2] + line[3:])), line[2]
...
feature_columns = [ tf.feature_column.numeric_column(i, dtype=tf.float32) for i in feature_names ]
...
classifier.train(input_fn=lambda: m_input_fn("final.csv"))
